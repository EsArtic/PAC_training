...
def compute_prob(prob_list):
    result = 0.
    for elem in prob_list:
        result += math.log(elem)
    return result
...
def predict_image_caption_beam_search(net, pad_img_feature, vocab_list, strategy, max_length=50):
    ...
    while beams_complete < len(beams):
        ...
        for beam_index, beam_log_prob, beam in \
            ...
            # find the most possible word
            expansions_inds = probs.argsort()[-beam_size:]

            if len(beams[beam_index]) > 0:
                print vocab_inds_to_sentence(vocab_list, beams[beam_index]) + ':'
            else:
                print 'Null:'

            # deliver the expansions by given k words
            for ind in expansions_inds:
                prob = probs[ind]
                extended_beam_log_prob = beam_log_prob + math.log(prob)

                print vocab_list[ind], prob, '------', extended_beam_log_prob

                # if len(beams[beam_index]) <= 4 and ind == 0:
                #     continue

                # print 'Log_prob: ' + str(extended_beam_log_prob)
        ...
        new_beams = [beams[e['prefix_beam_index']] + e['extension'] for e in expansions]
        new_beam_probs = [beam_probs[e['prefix_beam_index']] + e['prob_extension'] for e in expansions]
        beam_log_probs = [e['log_prob'] for e in expansions]
        beams_complete = 0
        for beam in new_beams:
            if beam[-1] == 0 or len(beam) >= max_length:
                beams_complete += 1
        beams, beam_probs = new_beams, new_beam_probs

    # print 'beams, beam_probs, log_p_per_word'
    print 'beams, beam_probs, log_p'
    for i in xrange(len(beams)):
        curr_prob = compute_prob(beam_probs[i]) # / len(beams[i])
        print beams[i], beam_probs[i], curr_prob
    print

    return [beams[0]], [beam_probs[0]] 
...
def predict_image_caption(net, pad_img_feature, vocab_list, strategy={'type': 'beam'}):
    ...
    for _ in range(num_samples):
        sample, samples_prob = sample_image_caption(net, pad_img_feature, strategy)
        samples.append(samples)
        sample_probs.append(sample_prob)
    return samples, sample_probs

def run_pred_iter(net, pad_img_feature, vocab_list, strategies=[{'type': 'beam'}]):
    outputs = []
    for strategy in strategies:

        # entrance of generation for video captions and words' probabilities
        captions, probs = predict_image_caption(net, pad_img_feature, vocab_list, strategy=strategy)
...
def run_pred_iters(pred_net, vidids, video_gt_pairs, fsg,
    ...
    for video_id in vidids:
        gt_captions = video_gt_pairs[video_id] # get the target stream
...
def main():
    ...
    FRAMEFEAT_FILE_PATTERN = '/home/share/data_from_author/{0}.txt'
    ...
    for split_name, data_split_name, aligned in DATASETS:
        ...
        for c in range(START_CHUNK, NUM_CHUNKS):
            ...
            print 'output type length: ' + str(len(text_out_types))
